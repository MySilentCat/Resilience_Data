...
def UML_Resilience_Assessment(UML_file, static_bayesian_network_path, dynamic_Bayesian_network_path):
    comp = comp_wsd_to_txt(UML_file)
    Dynamic_Bayesian_network_MAIN(comp, static_bayesian_network_path, dynamic_Bayesian_network_path)

# 基于源代码设计恢复的软件架构韧性评估
def Code_Resilience_Assessment(file_list, fileTypes, project_path, comp_num, static_bayesian_network_path, dynamic_Bayesian_network_path):
    comps, Attacked_nodes = comp_main(file_list, fileTypes, project_path, comp_num)
    categories = [
        {
    ...
    for i in range(len(comps)):
        ...
        static_json_data["data"].update(static_js_data["data"])
        static_json_data["links"].update(static_js_data["links"])
        static_json_data["resilience"].update(static_js_data["resilience"])
        Dynamic_json_data["data"].update(Dynamic_js_data["data"])
        Dynamic_json_data["links"].update(Dynamic_js_data["links"])
        Dynamic_json_data["resilience"].update(Dynamic_js_data["resilience"])

    with open(static_bayesian_network_path, 'w') as f:
        json.dump(static_json_data, f)
    with open(dynamic_Bayesian_network_path, 'w') as f:
        json.dump(Dynamic_json_data, f)
...
if __name__ == '__main__':
    ...
    Code_Resilience_Assessment(file_list, fileTypes, project_path, comp_num, static_bayesian_network_path, dynamic_Bayesian_network_path)
