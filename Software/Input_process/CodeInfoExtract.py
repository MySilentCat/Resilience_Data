...
import chardet
import copy
from Input_process.utils import *
from Input_process.merge_funcInfo import *
import networkx as nx
# import call_back
import Input_process.sdg_new
from Input_process.BuildCG import deal_pointer_func
        ...
        if file in change_file_list:
            ...
            with open(file, 'r', encoding="utf-8") as f:
                with open(new_file, 'w', encoding="utf-8") as f1:
                    for line in f.readlines():
                        f1.write(line)
            run_cmd('./Input_process/CPP_Parsing/build/CPP_Parsing ' + new_file)
            call_txt = new_file+".call"
            if not os.path.exists(call_txt):
                continue
            ...
            run_cmd('./Input_process/CPP_Parsing/build/CPP_Parsing ' + file)
...
def run_DDAnalysis(file_list, project_folder):
    ...
    for file in file_list:
        ...
        #                 f1.write(line)
        # 基于include来判定是否需要将该文件转换为.hpp文件
        if os.path.exists(file+'.var_define1'):
            continue
        if file in change_file_list:
            new_file = change_file_list[file]
            with open(file, 'r', encoding="utf-8") as f:
                with open(new_file, 'w', encoding="utf-8") as f1:
                    for line in f.readlines():
                        f1.write(line)
            run_cmd('./Input_process/DDAnalysis/build/DDAnalysis ' + new_file)
            var_define1 = file+'.var_define1'
            if not os.path.exists(var_define1):
                continue
            ...
            run_cmd('./Input_process/DDAnalysis/build/DDAnalysis ' + file)
    ...
    for file in file_list:
        ...
        for func1 in temp_func_list:
            ...
            for func2 in fileinfo['functionList']:
                if key == func2[0]:
                    funcInfo_dict[func2[-2]]['anno'].extend(func1.anno)
...
def GetInfo(file_list, project_path):
    ...
    for codefile in file_list:
        ...
        for funcInfo_str in funcInfo_list:
            ...
            for i in range(len(funcInfo_str_list)):
                ...
                if i == 0:
                    ...
                    locateFile = infos[0]
                    locateFile = deal_path(locateFile)
                    print(locateFile)
                    funcName = infos[2]
                    ...
                    #             break
                    locateFile_path = ''
                    if locateFile.split(':')[0] not in file_list:
                        print('codefile',codefile)
                        locateFile = codefile+':'+locateFile.split(':')[1]
                        locateFile_path = codefile
                    else:
                        locateFile_path = locateFile.split(':')[0]
                        locateFile = locateFile_path+':'+locateFile.split(':')[1]
                    print(locateFile)
                    isbody = infos[1]
                    funcName = infos[2]
                    funcClass = infos[3]
                    ...
                    # if './' in first_func_key:
                    #     print(first_func_key)
                    #     raise first_func_key
                    
                    first_func_key = first_func_key.replace('[','').replace(']','')
                    print('first_func_key: ',first_func_key)
                    ...
                    #locateFile = deal_path(locateFile)
                    if locateFile.split(':')[0] not in file_list:
                        file_name = locateFile.split(':')[0].split('/')[-1]
                        for file in file_list:
                            temp_file_name = file.split('/')[-1]
                            if file_name == temp_file_name:
                                locateFile = file+':'+locateFile.split(':')[-2]+':'+locateFile.split(':')[-1]
                                break
                    funcName = infos[2]
                    # 去掉最后一个冒号之后的字符串
                    ...
                    if call_loc.split(':')[0] not in file_list:
                        call_loc = codefile+':'+call_loc.split(':')[1]
                    print(call_loc)
                    ...
                    if locateFile != '<invalid loc>' and locateFile[0] != '':
                        ...
                        if funcInfo_dict[first_func_key].startLine <= call_line and funcInfo_dict[first_func_key].endLine >= call_line:
                            funcInfo_dict[first_func_key].fanOut.append([func_key, call_loc, 0])
                        else:
                            func_key_list = []
                            for key, value in funcInfo_dict.items():
                                if value.startLine <= call_line and value.endLine >= call_line:
        ...
        for funcParam_str in funcParam_list:
            ...
            total_code_line, vaild_code_line, total_comment_line, vaild_comment_line, comment_text = code_comment_line(codeText[start_line-1:end_line])
            code = ' '.join(codeText[start_line-1:end_line]).replace('\n', ' ').replace('\t', '')
            # code_summarization = run_codet5p_bimodal(code)
            codeInfo = {}
            ...
            codeInfo['comment_text'] = comment_text
            ...
            # if func_key not in funcInfo_dict.keys():
            #     func_key = matched_func_key[func_key]
            func_key = func_key.replace('[','').replace(']','')
            if func_key not in funcInfo_dict.keys():
                continue
            ...
            if_body = funcInfo_dict[func_key].ifbody
            # if if_body == '1':
            #     funcInfo_dict[func_key].code_summarization = run_codet5p_bimodal(code)
            params = param_info[5:]
            for param in params:
                if param == '':
                    continue
        ...
        # 获取文件代码信息
        total_code_line, vaild_code_line, total_comment_line, vaild_comment_line, comment_text = code_comment_line(codeText)
        codeInfo = {}
        codeInfo['codeLine'] = total_code_line
        codeInfo['codeLineExp'] = vaild_code_line
        codeInfo['commentLine'] = total_comment_line
...
def saveasdict(codeFileInfo_dict, funcInfo_dict, classInfo_dict, global_var_list):
    ...
    for key, value in codeFileInfo_dict.items():
        ...
        for func in value.functionList:
            ...
            func_temp.append(func_info.key.replace('[','').replace(']',''))
    ...
    for key, value in funcInfo_dict.items():
        ...
        new_funcInfo_dict[key]['anno'] = value.codeInfo['comment_text']
        new_funcInfo_dict[key]['modify'] = list(set(value.modify))
        new_funcInfo_dict[key]['code_summarization'] = value.code_summarization

        new_funcInfo_dict[key]['start_line'] = value.startLine
        new_funcInfo_dict[key]['end_line'] = value.endLine

        new_funcInfo_dict[key]['variables'] = []

        for var in value.variableList:
            new_funcInfo_dict[key]['variables'].append(var.name)
        fanOut = []
        fanout_callLine = {}
        for fan in value.fanOut:
            if fan[2] == 0 and fan[0] not in fanOut:
                fanOut.append(fan[0])
                if fan[0] not in fanout_callLine.keys():
                    fanout_callLine[fan[0]] = [fan[1]]
                else:
                    fanout_callLine[fan[0]].append(fan[1])
        new_funcInfo_dict[key]['fanout_callLine'] = fanout_callLine
...
def build_plcg_dot(new_funcInfo_dict, project_path, funcInfo_dict):
            ...
            for fan in funcInfo_dict[source_key].fanOut:
                ...
                for node in PLCG_nx.nodes:
                    ...
                    if fan_key in node_label:
                        ...
                        break
                if find == 0:
                    print('fan_key', fan_key)
                    file = '"'+":".join(fan_key.split(":")[:-1]) + '"'
                    if fan[2] == 1:
                        lib_file = '"' + fan_key.split(":")[0].split("/")[-1] + '"'
    ...
    for key, value in new_funcInfo_dict.items():
        for fan in value['fanOut']:
            fan_key = fan
            if fan_key in id_label_match.keys():
                PLCG_nx.add_edge(id_label_match[key], id_label_match[fan_key])
            elif '->' in fan_key:
                # 未处理的函数指针
                print(fan_key)
                # raise ValueError
                pf_key = ":".join(fan_key.split(":")[2:]).replace("->", ".")
                if pf_key == 'sudo_fn_execve_t':
                    raise ValueError
                pf_file = ':'.join(fan_key.split(":")[2:3])
                start_line = funcInfo_dict[key].startLine
                func_code = new_funcInfo_dict[key]['codeText']
                func_name = pf_key
                call_code = ""
                call_code += func_name + ": "
                for call_line in new_funcInfo_dict[key]['fanout_callLine'][fan_key]:
                    call_line = int(call_line.split(":")[-1])
                    print(call_line - start_line)
                    print(func_code)
                    if call_line - start_line < len(func_code) and call_line - start_line>=0:
                        call_code += func_code[call_line - start_line].replace("\n", "").replace('\t', '') + "@@@"
                ...
                print('key', key)
                print('fan_key', fan_key)
                print(id_label_match)
                PLCG_nx.add_node(id, label=label, Lib="", file='"' + pf_file + '"', call_code='"'+call_code+'"')
                PLCG_nx.add_edge(id_label_match[key], id_label_match[fan_key])

                fun_info = {}
                fun_info['name'] = pf_key.split('$$')[-1]
                fun_info["locateFile"] = pf_file
                fun_info['codeText'] = []
                fun_info['anno'] = []
                fun_info['variables'] = []
                fun_info['codeInfo'] = {}
                fun_info['codeInfo']['codeLine'] = 0
                fun_info['codeInfo']['codeLineExp'] = 0
                fun_info['codeInfo']['commentLine'] = 0
                fun_info['codeInfo']['commentLineExp'] = 0
                fun_info['codeInfo']['funcLine'] = 0
                fun_info['codeInfo']['comment_text'] = []
                fun_info['paramList'] = []
                fun_info['returnType'] = []
                fun_info['fanIn'] = []
                fun_info['fanOut'] = []
                fun_info['header'] = pf_file
                fun_info['source'] = pf_file
                
                mew_funInfo[pf_key] = fun_info
            else:
                pf_key = fan_key
                pf_file = fan_key.split(":")[0]
                start_line = funcInfo_dict[key].startLine
                func_code = new_funcInfo_dict[key]['codeText']
                func_name = pf_key
                call_code = ""
                call_code += func_name + ": "
                if pf_key not in new_funcInfo_dict.keys():
                    continue
                for call_line in new_funcInfo_dict[key]['fanout_callLine'][fan_key]:
                    call_line = int(call_line.split(":")[-1])
                    print(call_line - start_line)
    ...
    new_funcInfo_dict.update(mew_funInfo)

    return PLCG_nx, new_funcInfo_dict
...
def main(file_list, fileTypes, project_path):
    ...
    run_DDAnalysis(file_list, project_path)
    # 获取信息
    ...
    # 处理CG
    #new_funcInfo_dict = deal_pointer_func(new_funcInfo_dict, project_path)

    # 将plcg存到nx中
    PLCG_nx, new_funcInfo_dict = build_plcg_dot(new_funcInfo_dict, project_path, funcInfo_dict)
    ...
    # sdg
    sdg_svg, sdg_nx = Input_process.sdg_new.main(file_list, PLCG_nx, project_path)
    ...
    return new_funcInfo_dict, PLCG_nx, sdg_nx  # , call_back_json, layer_graph_json


if __name__ == "__main__":
    project_path = r"\temp_grad\Input_process\test\saolei".replace("\\","/")
    file_list, fileTypes = getfilelist(project_path)
    new_funcInfo_dict, PLCG_nx, sdg_nx = main(file_list, fileTypes,project_path)
