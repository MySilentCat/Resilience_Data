    ...
    <el-container style="height: 88vh; border: 1px solid #eee">
      <el-aside>
        <div class="grid-content">
          <el-tree :data="treeData" :props="treeProps" node-key="id" max-depth="1" highlight-current
      ...
      <el-main style="height: 100%">
        <div style="height: 38px; " v-if="this.metricSelected !==0">
          ...
          <el-button type="primary" style="float: right;margin-right: 20px;" size="medium"
                     @click="toResult">下一步
          </el-button>
          <el-button type="primary" style="float: right;margin-right: 10px;" size="medium"
                     @click="toCheck">校验
          </el-button>
          ...
          <el-dialog title="权重说明" :visible.sync="dialogTableVisible">
            ...
            <p style="margin-top: 5%; font-size:16px"><strong>应用示例:</strong>功能适应性:性能效率<br/>
              （1）如果权值为1:5，表示<strong>性能效率</strong>相对于<strong>功能适应性</strong><span
                  style="color: #fdd381">较强重要</span><br/>
              （2）如果权值为5:1，表示<strong>功能适应性</strong>相对于<strong>性能效率</strong><span
          ...
          <div v-for="(tdata,index) in tableData" :key="index">
            ...
            >
              <template slot-scope="scope">
                ...
                <el-select v-model="weightMatrix[scope.$index][index]"
...
export default {
  ...
  methods: {
    ...
    toCheck() {
      ...
      utils.axiosMethod({
        method: "POST",
        url: config.BASE_URL + "/weightCheck/" + _this.$store.state.current_project.name,
        data: {
  ...
  mounted() {
    ...
    utils.axiosMethod({
      method: "POST",
      url: config.BASE_URL + "/metricSelected/" + _this.$store.state.current_project.name,
      data: {
      ...
      callback: (res) => {
        ...
        utils.axiosMethod({
          method: "GET",
          url: config.BASE_URL + "/metricWeight/" + _this.$store.state.current_project.name,
          callback: (res) => {
