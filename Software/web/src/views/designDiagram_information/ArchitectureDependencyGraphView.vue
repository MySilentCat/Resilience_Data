        ...
        ><el-card class="box-card" style="height: 100%">
          <div slot="header" class="clearfix">
            ...
            <el-popover placement="right" trigger="click" style="margin-left: 5px">
              <div>
                &bull;
                圆角矩形节点表示组件，圆形节点表示函数，实线表示控制依赖关系，虚线表示数据依赖关系
              </div>
              <div>
                &bull; 双击组件节点可将该组件展开成函数，双击函数节点可恢复到组件节点
              </div>
          ...
          <p id="intro_text">
            系统架构图是在系统依赖图的基础上恢复的，图中的节点是系统中的组件，组件是由一组紧密关联的函数组成，边表示组件之间的依赖关系。它提供了系统组件架构的简明描述，以协助组件与组件之间的关系和系统的运作。系统架构图是软件架构的一个可视化表示，可以清晰地表示系统中不同组件之间的关系，以便系统设计者和软件工程师更好地了解系统的架构。其中虚线表示数据依赖，实线表示控制依赖。
          </p>
        </el-card></el-col
      >
    </el-row>
    ...
    <el-row style="height: 80%; width: 100%">
      ...
      <el-col :span="6" style="height: 100%">
        <el-card class="box-card" style="width: 99%; height: 50%" shadow="hover">
          ...
          <div ref="number_of_funcs" class="text item">组件包含函数个数：</div>
...
export default {
  ...
  data() {
    return {
      ...
      url: "/graphs/clustered_sdg.dot_GNwithoutcomp.dot.html",
    };
  },
  mounted() {
    // 接受iframe组件传过来的数据
    ...
    // console.log("%%%%%%", params);
    if (params.title == "架构依赖图") {
      this.url = "/graphs/clustered_sdg.dot_GNwithoutcomp.dot.html";
    } else {
