<template>
    <el-breadcrumb id ="nav-top" separator-class="el-icon-arrow-right" class="nav-top">
        <el-breadcrumb-item v-if="!isCreate" :to="{path: '/home'}">BNSA.Resilience Evaluator</el-breadcrumb-item>
...
export default {
    ...
    watch:{
        $route(to, from) {
            ...
            const matched_list = ["create", "selectDoc"]
            this.isCreate = matched_list.indexOf(matched[0].name) != -1 ? true : false

            if(matched[0].meta.title == "BNSA.Resilience Evaluator") {
                this.naviList = []
            }
